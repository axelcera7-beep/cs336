...
from cs336_basics.model import Linear, Embedding, RMSNorm, PositionWiseFeedForward, RoPE, CausalMultiHeadAttention, TransformerBlock, Transformer
from cs336_basics.utils import cross_entropy, learning_rate_schedule, gradient_clipping, data_loading, save_checkpoint, load_checkpoint, softmax, scaled_dot_product_attention
...
def run_swiglu(
    ...
    swiglu = PositionWiseFeedForward(d_model=d_model, d_ff=d_ff, device=w1_weight.device, dtype=w1_weight.dtype)
...
def run_multihead_self_attention(
    ...
    multihead_self_attention = CausalMultiHeadAttention(d_model=d_model, num_heads=num_heads)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multihead_self_attention = CausalMultiHeadAttention(d_model=d_model, num_heads=num_heads, theta=theta)
    multihead_self_attention.q_proj.weight.data = q_proj_weight
    multihead_self_attention.k_proj.weight.data = k_proj_weight
    multihead_self_attention.v_proj.weight.data = v_proj_weight
    multihead_self_attention.output_proj.weight.data = o_proj_weight
...
def run_transformer_block(
    ...
    """
    transformer_block = TransformerBlock(d_model=d_model, num_heads=num_heads, d_ff=d_ff, theta=theta)
...
def run_transformer_lm(
    ...
    """
    transformer = Transformer(vocab_size=vocab_size, context_length=context_length, num_layers=num_layers, theta=rope_theta, d_ff=d_ff, num_heads=num_heads, d_model=d_model)
...
def run_silu(in_features: Float[Tensor, " ..."]) -> Float[Tensor, " ..."]:
    ...
    return PositionWiseFeedForward.silu(x=in_features)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return learning_rate_schedule(t=it, alpha_min=min_learning_rate, alpha_max=max_learning_rate, T_w=warmup_iters, T_c=cosine_cycle_iters)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    dict, merges = train_bpe(input_path, vocab_size, special_tokens)
    return dict, merges
